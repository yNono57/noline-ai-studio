...
import { GitHubOnboardingError } from "@/lib/forge/github-onboarding";
...
export function githubErrorResponse(error: unknown) {
  ...
  if (error instanceof GitHubOnboardingError) return NextResponse.json({ error: error.message }, { status: error.code === "INVALID_ORIGIN" ? 403 : 400 });
