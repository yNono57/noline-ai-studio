...
export function ForgeAgentRunnerPanel({ conversationId, launchRequest, onLaunchRequestHandled, onActiveChange, onConversationUpdated, onPayloadChange, onMessagesPersisted, onLaunchSettled }: { conversationId: string; launchRequest?: LaunchRequest | null; onLaunchRequestHandled?: (id: string) => void; onActiveChange?: (active: boolean) => void; onConversationUpdated?: (conversation: ForgeConversation) => void; onPayloadChange?: (payload: ForgeAgentRunPayload | null) => void; onMessagesPersisted?: (messages: ForgeMessage[]) => void; onLaunchSettled?: (submissionId: string) => void }) {
  ...
  const launchObjective = useCallback(async (value: string, submissionId = crypto.randomUUID()) => { if (!value.trim() || active || launchInFlight.current) return; launchInFlight.current = true; setObjective(value); setLaunching(true); setError(""); setDiff(""); try { const result = await startForgeAgentRun(conversationId, value.trim(), submissionId); setPayload(result.agentRun); onPayloadChange?.(result.agentRun); onConversationUpdated?.(result.conversation); onMessagesPersisted?.([result.user_message, ...(result.assistant_message ? [result.assistant_message] : [])]); } catch (caught) { setError(caught instanceof Error ? caught.message : "Run Forge indisponible."); } finally { launchInFlight.current = false; setLaunching(false); onLaunchSettled?.(submissionId); } }, [active, conversationId, onConversationUpdated, onLaunchSettled, onMessagesPersisted, onPayloadChange]);
