...
export function ForgeAgentRunnerPanel({ conversationId, launchRequest, onLaunchRequestHandled, onActiveChange, onConversationUpdated }: { conversationId: string; launchRequest?: LaunchRequest | null; onLaunchRequestHandled?: (id: string) => void; onActiveChange?: (active: boolean) => void; onConversationUpdated?: (conversation: ForgeConversation) => void }) {
  ...
  useEffect(() => { let mounted = true; const refresh = () => getLatestForgeAgentRun(conversationId).then(({ agentRun }) => { if (mounted) setPayload(agentRun); }).catch(() => undefined); void refresh(); if (!active) return () => { mounted = false; }; const timer = window.setInterval(refresh, 1500); return () => { mounted = false; window.clearInterval(timer); }; }, [active, conversationId, onConversationUpdated]);
  ...
  const launchObjective = useCallback(async (value: string) => { if (!value.trim() || active) return; setObjective(value); setLaunching(true); setError(""); setDiff(""); try { const result = await startForgeAgentRun(conversationId, value.trim()); setPayload(result.agentRun); onConversationUpdated?.(result.conversation); } catch (caught) { setError(caught instanceof Error ? caught.message : "Run Forge indisponible."); } finally { setLaunching(false); } }, [active, conversationId, onConversationUpdated]);
