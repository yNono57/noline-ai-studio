...
import { ForgeArtifactHistory } from "@/components/ForgeArtifactHistory";
  ...
  return <div className="min-w-0">
    ...
    <div className={`grid min-w-0 gap-3 ${workspaceOpen ? "xl:grid-cols-[16rem_minmax(0,1fr)_23rem]" : "xl:grid-cols-[16rem_minmax(0,1fr)]"}`}>
      ...
      <ForgeWorkspaceDrawer open={workspaceOpen} title={conversation?.title || project?.name || "Forge"} onClose={() => setWorkspaceOpen(false)}>
        ...
        <ForgeArtifactHistory conversationId={conversationId} runtimeReady={runtimeReady} />
...
function ForgeAgentActivity({ payload, diffSummary, onOpenDiff }: { payload: ForgeAgentRunPayload; diffSummary: { files: number; additions: number; deletions: number } | null; onOpenDiff: () => void }) {
  ...
  const artifactSummary = payload.artifact ? { files: payload.artifact.changedFiles.length, additions: payload.artifact.additions, deletions: payload.artifact.deletions } : diffSummary;
  ...
  return <article className="max-w-[96%] rounded-xl border border-noline-orange/25 bg-noline-orange/5 px-4 py-3 text-white sm:max-w-[88%]">
    ...
    {payload.run.status === "COMPLETED" && payload.artifact ? <p className="mt-3 text-xs font-black text-green-300">Changements sauvegardés</p> : null}
    {payload.run.status === "COMPLETED" && artifactSummary && artifactSummary.files > 0 ? <button type="button" onClick={onOpenDiff} className="mt-2 flex min-h-11 w-full items-center justify-between rounded-lg border border-white/10 bg-black/20 px-3 text-left text-xs text-white hover:border-noline-orange/40"><span><strong>{artifactSummary.files} fichier(s) modifié(s)</strong><span className="ml-2 font-mono text-green-300">+{artifactSummary.additions}</span><span className="ml-1 font-mono text-red-300">-{artifactSummary.deletions}</span></span><span className="font-black text-noline-orange">Voir les changements</span></button> : null}
  </article>;
}
