...
import { cancelAgentRun, createAgentRun, createAgentRunArtifact, getAgentRun, getAgentRunArtifact, getLatestAgentRun, insertAgentStep, listAgentRuns, isAgentRunCancelled, listAgentSteps, updateAgentRun, updateAgentStep } from "./agent-store";
...
export const forgeAgentRunner = createForgeAgentRunner({
  resolveContext: readyContext, createRun: createAgentRun, updateRun: updateAgentRun, appendStep: insertAgentStep, updateStep: updateAgentStep, createArtifact: createAgentRunArtifact, isCancelled: isAgentRunCancelled,
...
export async function getForgeAgentRunView(userId: string, conversationId: string, runId?: string) { const workspace = await ownedWorkspace(userId, conversationId); const run = runId ? await getAgentRun(userId, runId) : await getLatestAgentRun(userId, conversationId); if (!run || run.conversationId !== conversationId || run.workspaceId !== workspace.workspaceId) return null; return { run: publicAgentRun(run), steps: await listAgentSteps(userId, run.runId), artifact: await getAgentRunArtifact(userId, run.runId) }; }
export async function listForgeAgentRunViews(userId: string, conversationId: string) { const workspace = await ownedWorkspace(userId, conversationId); const runs = (await listAgentRuns(userId, conversationId)).filter((run) => run.workspaceId === workspace.workspaceId); return Promise.all(runs.map(async (run) => ({ run: publicAgentRun(run), steps: await listAgentSteps(userId, run.runId), artifact: await getAgentRunArtifact(userId, run.runId) }))); }
